...
function makeItem(selectedIndex, i, anchor) {
    ...
    return <li key={i} ref={isSelected ? "selected" : null} className={className}>{anchor}</li>;
}
    ...
    render: function () {
        ...
        if (props.isLoading) {
            ...
            files = files.map(function (link, i) {
                return makeItem(idx, i + directories.length,
                    <a
                        href={link.url}
